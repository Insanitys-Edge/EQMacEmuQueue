...
setup_queue_environment() {
    ...
    # Enable queue system
    set_rule "World.EnableQueue" "true"
    set_rule "World.EnableQueueLogging" "true"
    set_rule "World.QueueEstimatedWaitPerPlayer" "30"
...
set_population_offset() {
    ...
    local current_offset=$(mysql -h"$DB_HOST" -u"$DB_USER" -p"$DB_PASS" "$DB_NAME" -sN -e "SELECT rule_value FROM rule_values WHERE rule_name IN ('World.TestPopulationOffset', 'World:TestPopulationOffset') LIMIT 1;" 2>/dev/null)
    ...
    execute_sql "UPDATE rule_values SET rule_value='$new_offset' WHERE rule_name='World.TestPopulationOffset';"
    execute_sql "UPDATE rule_values SET rule_value='$new_offset' WHERE rule_name='World:TestPopulationOffset';"
    
    # Create the rule if it doesn't exist
    execute_sql "INSERT IGNORE INTO rule_values (ruleset_id, rule_name, rule_value, notes) VALUES (1, 'World:TestPopulationOffset', '$new_offset', 'Test population offset for queue testing');"
...
live_queue_monitor() {
    ...
    while true; do
        ...
        local test_offset=$(mysql -h"$DB_HOST" -u"$DB_USER" -p"$DB_PASS" "$DB_NAME" -sN -e "SELECT rule_value FROM rule_values WHERE rule_name = 'World:TestPopulationOffset';" 2>/dev/null)
        ...
        local queue_enabled=$(mysql -h"$DB_HOST" -u"$DB_USER" -p"$DB_PASS" "$DB_NAME" -sN -e "SELECT rule_value FROM rule_values WHERE rule_name = 'World:EnableQueue';" 2>/dev/null)
        
        # If colon notation doesn't work, try dot notation as fallback
        if [ -z "$test_offset" ]; then
            test_offset=$(mysql -h"$DB_HOST" -u"$DB_USER" -p"$DB_PASS" "$DB_NAME" -sN -e "SELECT rule_value FROM rule_values WHERE rule_name = 'World.TestPopulationOffset';" 2>/dev/null)
        fi
        ...
        if [ -z "$queue_enabled" ]; then
            queue_enabled=$(mysql -h"$DB_HOST" -u"$DB_USER" -p"$DB_PASS" "$DB_NAME" -sN -e "SELECT rule_value FROM rule_values WHERE rule_name = 'World.EnableQueue';" 2>/dev/null)
        fi
        ...
        if [ "$rule_count" -gt 0 ]; then
            mysql -h"$DB_HOST" -u"$DB_USER" -p"$DB_PASS" "$DB_NAME" -sN -e "
                SELECT 
                    CONCAT('  ', 
                        CASE 
                            WHEN rule_name = 'World.EnableQueue' THEN 'Queue Enabled: '
                            WHEN rule_name = 'Quarm.PlayerPopulationCap' THEN 'Max Players: '
                            WHEN rule_name = 'World.TestPopulationOffset' THEN 'Test Offset: '
                            WHEN rule_name = 'World.QueueBypassGMLevel' THEN 'GM Bypass: '
                            WHEN rule_name = 'World.QueueEstimatedWaitPerPlayer' THEN 'Wait Per Player: '
                            WHEN rule_name = 'World.EnableQueueLogging' THEN 'Debug Logging: '
                            ELSE CONCAT(SUBSTRING(rule_name, 7), ': ')
                        END,
                        rule_value,
                        CASE 
                            WHEN rule_name = 'World.QueueEstimatedWaitPerPlayer' THEN ' seconds'
                            ELSE ''
                        END
                    )
                FROM rule_values 
    ...
    echo -e "${PURPLE}  Queue mode: (1) Dynamic - Rotate RAID, NORM, GRP, NEWB | ${ORANGE}**(2) Chrono - Time based**${PURPLE}${NC}"
        ...
        case "$key" in
            ...
            "p"|"P")
                ...
                echo
                local current_offset=$(mysql -h"$DB_HOST" -u"$DB_USER" -p"$DB_PASS" "$DB_NAME" -sN -e "SELECT rule_value FROM rule_values WHERE rule_name = 'World:TestPopulationOffset' LIMIT 1;" 2>/dev/null)
                current_offset=${current_offset:-0}
                ...
                if [[ "$new_offset" =~ ^[0-9]+$ ]] && [ "$new_offset" -ge 0 ] && [ "$new_offset" -le 9999 ]; then
                    mysql -h"$DB_HOST" -u"$DB_USER" -p"$DB_PASS" "$DB_NAME" -e "
                        ...
                        VALUES (1, 'World:TestPopulationOffset', '$new_offset')
                        ON DUPLICATE KEY UPDATE rule_value = '$new_offset';
                    " 2>/dev/null
                    
                    # Verify the update
                    local verified_offset=$(mysql -h"$DB_HOST" -u"$DB_USER" -p"$DB_PASS" "$DB_NAME" -sN -e "SELECT rule_value FROM rule_values WHERE rule_name = 'World:TestPopulationOffset';" 2>/dev/null)
                    
                    notification="${GREEN}Population offset set: $current_offset → $new_offset${NC}"
                    notification+="\n${BLUE}Verified in database: World:TestPopulationOffset = $verified_offset${NC}"
...
cleanup() {
    ...
    if [ -n "$world_server_id" ]; then
        ...
        set_rule "World.EnableQueue" "true"
        set_rule "World.TestPopulationOffset" "0"
        set_rule "World.QueueBypassGMLevel" "true"
        set_rule "World.QueueEstimatedWaitPerPlayer" "60"
        set_rule "World.EnableQueueLogging" "true"
...
toggle_gm_bypass() {
    ...
    local current_gm_bypass=$(mysql -h"$DB_HOST" -u"$DB_USER" -p"$DB_PASS" "$DB_NAME" -sN -e "SELECT rule_value FROM rule_values WHERE rule_name = 'World:QueueBypassGMLevel';" 2>/dev/null)
    ...
    set_rule "World:QueueBypassGMLevel" "$new_gm_bypass"
...
toggle_freeze_queue() {
    ...
    local current_freeze=$(mysql -h"$DB_HOST" -u"$DB_USER" -p"$DB_PASS" "$DB_NAME" -sN -e "SELECT rule_value FROM rule_values WHERE rule_name = 'World:FreezeQueue';" 2>/dev/null)
    ...
    set_rule "World:FreezeQueue" "$new_freeze"
...
# Function to trigger rule reload on world server  
trigger_rule_reload() {
    # The world server checks for ServerOP_ReloadRules packets
    # We can simulate this by updating a timestamp that forces a rule reload
    mysql -h"$DB_HOST" -u"$DB_USER" -p"$DB_PASS" "$DB_NAME" -e "
        INSERT INTO tblloginserversettings (type, value, description) 
        VALUES ('ForceRuleReload', UNIX_TIMESTAMP(), 'Force world server to reload rules')
        ON DUPLICATE KEY UPDATE value = UNIX_TIMESTAMP();" 2>/dev/null
    
    if [ $? -eq 0 ]; then
        print_info "Rule reload signal sent - world server should pick up changes within 5 seconds"
    else
        print_error "Failed to send rule reload signal"
    fi
}
